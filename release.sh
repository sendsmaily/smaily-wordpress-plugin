...
BUILD_URL="https://github.com/sendsmaily/smaily-wordpress-plugin/releases/download/${VERSION}/${PLUGIN_SLUG}.zip"
...
curl -L --output "/tmp/${PLUGIN_SLUG}-${VERSION}.zip" "${BUILD_URL}" || { echo "Unable to fetch build archive"; exit 1; }
unzip "/tmp/${PLUGIN_SLUG}-${VERSION}.zip" -d /tmp
