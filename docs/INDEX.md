...
| `MIGRATION.md` | WP-admin running the wordpress.org `smaily-connect` 2.0.0 package | **Written** (`docs/MIGRATION.md`) | How to upgrade 2.0.0 → the current 3.x release: the directory's own update, what persists, wizard takeover, rollback path. (This row previously said TODO after the doc had shipped — fixed 2026-06-11.) |
...
| `CHANGELOG.md` | Anyone following an old link | **Retired** (2026-09-10, PRO-2394) | A pointer only. The changelog is `readme.txt`'s `== Changelog ==` section (what merchants and wordpress.org read) plus the GitHub release notes; the file's old content (up to 3.5.0) is in git history. |
