...
| 2. Durable opt-out registry | Single option (`smly_profiling_optouts`, autoload=false), keyed by hashed email, opt-outs only | No expiry, immune to transient/object-cache eviction | On a fresh-cache miss with a read error (or no Smaily client configured): checked FIRST. If the email is in the registry, profiling is denied — unconditionally, regardless of any stale value. Only an **explicit** opt-in removes the entry — a successful read-back of `smaily_rec_profiling = 1`, or the shopper opting back in from My Account; an error can never clear it, and neither can a successful read of a contact with no preference or no contact at all (PRO-3191: that is an opt-out whose Smaily write never landed; a found contact gets the opt-out written to it). |
