...
$sync_fields = get_option( Smaily_Options::SUBSCRIBER_SYNC_FIELDS_OPTION, Smaily_Options::SUBSCRIBER_SYNC_DEFAULT_FIELDS );
$labels      = array(
...
<fieldset >
	<?php foreach ( $sync_fields  as $field => $enabled ) : ?>
		<label for="<?php echo sprintf( '%s[%s]', esc_attr( Smaily_Options::SUBSCRIBER_SYNC_FIELDS_OPTION ), esc_attr( $field ) ); ?>">
			<input
				...
				name="<?php echo sprintf( '%s[%s]', esc_attr( Smaily_Options::SUBSCRIBER_SYNC_FIELDS_OPTION ), esc_attr( $field ) ); ?>"
				value="1"
