 ...
 */
class Smaily_Woocommerce_Admin
{
    ...
    public function save_api_information()
    {
        ...
        // Verify advanced form data

        if (isset($payload['advanced-form']['is_advanced']) && empty($payload['advanced-form']['form'])) {
            echo wp_json_encode(
                array(
                    'error' => __('Please fill out the area for the advanced form under "Newsletter" or switch off the advanced option!', 'smaily'),
                )
            );
            wp_die();
        }

        // Update advanced form data

        $newsletter = array_merge(['form' => '', 'is_advanced' => ''], $payload['advanced-form']);

        $newsletter['form'] = esc_textarea($newsletter['form']);
        $newsletter['is_advanced'] = (bool)$newsletter['is_advanced'];

        $ups = update_option('smailyforwp_form_option', $newsletter);
        //wp_send_json_error([$newsletter,$ups]);
    ...
    {
        return 'smaily-for-woocommerce/' . SMAILY_PLUGIN_VERSION . ' (WordPress/' . get_bloginfo('version') . ';' . (defined('WC_VERSION') ? ' WooCommerce/' . WC_VERSION . '; +' . get_bloginfo('url') . ')' : '');
    }
}
